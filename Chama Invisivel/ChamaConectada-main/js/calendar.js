...
jQuery(function($) {
  ...
  function createEventForm(id) {
    $('#evt')
      .append($('<div>').addClass('ae')
        .append($('<div>').addClass('aetitle').html('Marque um dia'))
        .append($('<form>').submit(function() { return false; })
          ...
          .append($('<input>').attr('type', 'button').attr('value', 'Agendado').attr('id', 'addevent'))
        )
      )
  }
  ...
  function showDailyEvents(y, m, d) {
    ...
    if(events[y + '-' + m]) {
      ...
      i = 0;
      if(dailyEvents) {
        $('#de').append($('<div>').addClass('detitle').html('Maque o dia para realizar a doação'));
      }
